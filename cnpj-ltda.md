...
```mermaid
flowchart TD
    A[Definir Contrato Social] --> B[Consulta de Viabilidade - REDESIM]
    B --> C[Obter Certificado Digital e-CPF]
    C --> D[Elaborar Contrato Social com Contador]
    D --> E[Registrar na Junta Comercial]
    E --> F[Obter NIRE]
    F --> G[Inscricao CNPJ - Receita Federal]
    G --> H{Atividade requer IE?}
    H -- Sim --> I[Inscricao Estadual - SEFAZ]
    H -- Nao --> J[Inscricao Municipal - CCM]
    I --> J
    J --> K[Alvara de Funcionamento]
    K --> L{Licencas especificas?}
    L -- Sim --> M[Vigilancia / Bombeiros / Ambiental]
    L -- Nao --> N[Cadastro eSocial/INSS]
    M --> N
    N --> O[Escolher Regime Tributario]
    O --> P[Abrir Conta PJ]
    P --> Q[Empresa Ativa]
```
